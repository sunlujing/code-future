 ...
 * ===================================
 *
 * 比较有代表性的一题，通常的动态规划，都是 从 i -》 i+1,  因为 这个 越小的子 问题是最容易求解答的。
 * 但是这道题目恰恰是相反的，第一天的决策，要依赖后面决策。 是最复杂的。
 * 相反如果是 从最后一条开始，往前推导，则是自简单的， 所以 ，最简单的 子问题 是  n， 然后一次推导 n-1;
 *
 * d[n] 为 第 N 天开始，所需要的最小 的cost， 那么
 * d[n] = min(c[0]+d[n+1], c[1]+d[n+7], c[2]+d[n+30])
 *
 *
 */
